	...
	public boolean isPpsExist(String pps, long currentByteStart) {
	    ...
	    try {
	        while (currentByte < input.length()) {
	            if (shouldSkipRecord(currentByte, currentByteStart)) {
	                currentByte += RandomAccessEmployeeRecord.SIZE;
	                continue;
	            }

	            RandomAccessEmployeeRecord record = readRecordAtPosition(currentByte);
	            if (record != null && record.getPps().trim().equalsIgnoreCase(pps)) {
	        ...
	        System.err.println("Error checking PPS existence: " + e.getMessage());
	    }

	    return false;
	}

	private boolean shouldSkipRecord(long currentByte, long currentByteStart) {
	    return currentByte == currentByteStart;
	}

	private RandomAccessEmployeeRecord readRecordAtPosition(long position) throws IOException {
	    RandomAccessEmployeeRecord record = new RandomAccessEmployeeRecord();
	    input.seek(position);
	    record.read(input);
	    return record;
